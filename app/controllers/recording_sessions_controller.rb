...
class RecordingSessionsController < ApplicationController
  ...
  def edit
    index
    @recording_session = RecordingSession.find(params[:id])
    @recording_sessions = if params[:artist_id]
                            Artist.find(params[:artist_id]).recording_sessions
                          #     @artist = Artist.find_by(id: params[:artist_id])
                          #  if @artist.nil?
                          #   redirect_to artists_path, alert: "Artist not found"
                          #  else
                          #   @recording_sessions = @artist.recording_sessions
                            ...
                            Engineer.find(params[:engineer_id]).recording_sessions
                          elsif params[:studio_id]
                            Studio.find(params[:studio_id]).recording_sessions
                          else
                            RecordingSession.all
                                   #    redirect_to action: 'index'
                                 end
  end
  ...
  def create_artist_from_name
    self.artist_name = new_artist_name unless new_artist_name.blank?
  end
